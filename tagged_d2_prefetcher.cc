...
void prefetch_access(AccessStat stat)
{
    ...
    Addr pf_addr2 = pf_addr + BLOCK_SIZE;
    /*
    ...
    if (stat.miss && !in_cache(pf_addr2) && !in_mshr_queue(pf_addr2)) {
        issue_prefetch(pf_addr2);
    }
    if(!stat.miss && get_prefetch_bit(stat.mem_addr)) {
        if (!in_cache(pf_addr) && !in_mshr_queue(pf_addr)) {
            issue_prefetch(pf_addr);
        }
        if (!in_cache(pf_addr2) && !in_mshr_queue(pf_addr2)) {
            issue_prefetch(pf_addr2);
        }
